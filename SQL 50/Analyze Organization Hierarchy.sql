...
*/
-- create table
CREATE TABLE Employees (employee_id INT,employee_name VARCHAR(100),manager_id INT,salary INT,department VARCHAR(50))
...
Aggregates AS (
    ...
    FROM Employees e
    LEFT JOIN Subordinates s ON e.employee_id = s.manager
...
SELECT 
    ...
    a.budget
FROM Hierarchy h
JOIN Aggregates a ON h.employee_id = a.employee_id
