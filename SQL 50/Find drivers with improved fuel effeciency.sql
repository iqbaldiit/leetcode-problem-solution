...
CREATE TABLE drivers (driver_id INT,driver_name VARCHAR(255))

CREATE TABLE trips (trip_id INT,driver_id INT,trip_date DATE,distance_km DECIMAL(8, 2),fuel_consumed DECIMAL(6, 2))
...
-- 	SELECT driver_id	
-- 	, (distance_km*1.00 / fuel_consumed) AS efficiency	
-- 	, CASE WHEN EXTRACT (MONTH FROM (trip_date)) BETWEEN 1 AND 6 THEN 1 ELSE 2 END AS half_year	
-- 	FROM trips 	
-- ), driver_effeciency_avg AS (
-- 	SELECT driver_id,half_year
-- 	,AVG(efficiency) avg_effeciency
