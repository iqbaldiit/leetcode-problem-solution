...
--Source (PostgreSQL): https://leetcode.com/problems/find-golden-hour-customers/solutions/7279217/simple-best-solution/
/*
...
--	SELECT customer_id
--	, COUNT(*) AS total_orders
--	,SUM(CASE WHEN (HOUR(order_timestamp) BETWEEN 11 AND 13)
...
WITH order_sum AS (
	SELECT customer_id
	, COUNT(*) AS total_orders
	,SUM(CASE WHEN ((EXTRACT (HOUR FROM CAST(order_timestamp AS TIMESTAMP))) BETWEEN 11 AND 13)
					OR ((EXTRACT (HOUR FROM CAST(order_timestamp AS TIMESTAMP))) BETWEEN 18 AND 20) 
		THEN 1 ELSE 0 END) AS peak_total
	,COUNT(order_rating) rated_orders
	,ROUND(AVG(order_rating*1.00),2) average_rating
	FROM restaurant_orders
