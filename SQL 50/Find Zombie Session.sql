...
-- Source (Oracle): https://leetcode.com/problems/find-zombie-sessions/solutions/7198155/simple-best-solution-by-iqbaldiit-xcwz/
-- Source (PostgreSQL): https://leetcode.com/problems/find-zombie-sessions/solutions/7195748/simple-best-solution/
...
*/
-- create table 
CREATE TABLE app_events (event_id INT,user_id INT,event_timestamp DATETIME,event_type VARCHAR(20),session_id VARCHAR(10),event_value INT)
...
--	FROM app_events A GROUP BY A.session_id, A.user_id
--), zombie_session AS (
--	SELECT Z.*,TIMESTAMPDIFF(MINUTE,Z.session_start,Z.session_end) AS duration FROM zombie Z
--)
--SELECT Z.session_id,Z.user_id,Z.duration AS session_duration_minutes, Z.scroll_count 
--FROM zombie_session Z
--WHERE duration>30 AND scroll_count>4 AND (click_count*1.00/scroll_count)<0.20 AND purchase_count<=0
...
-- 	FROM app_events A GROUP BY A.session_id, A.user_id
-- ), zombie_session AS (
-- 	SELECT Z.*, EXTRACT(EPOCH FROM (Z.session_end-Z.session_start)) / 60 AS duration FROM zombie Z    
-- )
-- SELECT Z.session_id,Z.user_id,Z.duration AS session_duration_minutes, Z.scroll_count 
-- FROM zombie_session Z
-- WHERE duration>30 AND scroll_count>4 AND (click_count*1.00/scroll_count)<0.20 AND purchase_count<=0
