...
--Source (MySQL, Oracle, PostgreSQL) :https://leetcode.com/problems/find-books-with-polarized-opinions/solutions/7120365/simple-best-solution-by-iqbaldiit-b47o/ 
/*
	Table: books
...
--Create table
CREATE TABLE books (book_id INT,title VARCHAR(255),author VARCHAR(255),genre VARCHAR(100),pages INT)
CREATE TABLE reading_sessions (session_id INT,book_id INT,reader_name VARCHAR(255),pages_read INT,session_rating INT)
...
--Solution (MSSQL, MySQL, Oracle, PostgreSQL)
WITH pol_op AS (
	SELECT R.book_id,COUNT(R.session_rating) AS sr_count 
	,MAX(R.session_rating) AS max_rating,MIN(R.session_rating) AS min_rating
	,SUM(CASE WHEN R.session_rating>=4 THEN 1 ELSE 0 END) AS rating_4
	,SUM(CASE WHEN R.session_rating<=2 THEN 1 ELSE 0 END) AS rating_2
	FROM reading_sessions R GROUP BY R.book_id
), pol_op2 AS (
	SELECT P.book_id
	,P.max_rating-P.min_rating AS rating_spread
	,ROUND((rating_4+rating_2)*1.00/sr_count*1.00,2) AS polarization_score 
	FROM pol_op P	
	WHERE sr_count>=5 AND rating_4>0 AND rating_2>0
)
SELECT PP.book_id,B.title,B.author,B.genre,B.pages
,PP.rating_spread,PP.polarization_score
FROM pol_op2 PP
